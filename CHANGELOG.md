...
- **`unifi_port_profile`: `tagged_networkconf_ids` no longer reports a successful apply for VLAN tagging the controller never applied.** The attribute targets the controller's legacy *inclusion* model (`forward = "customize"` plus a list of networks to tag), which newer firmware (observed on Network 10.6.x) replaced with an *exclusion* model (`tagged_vlan_mgmt = "custom"` plus `excluded_networkconf_ids`). Measured against the raw API with no Terraform involved, a POST carrying `forward: "customize"` and `tagged_networkconf_ids: [<id>]` returns HTTP 200 and the stored object reads back `forward = "all"` with no tagged networks — both fields are accepted and discarded. The provider compounded it: there is no go-unifi field for `tagged_networkconf_ids`, so the value never even left the process, and the read path unconditionally nulled it. Setting it is now rejected at plan time with an error naming the exclusion-based replacement, and the attribute is marked deprecated, instead of applying cleanly and doing nothing — the same treatment `unifi_network.ipv6_aliases` got in #413. `forward` stays writable for older controllers, but an explicit `forward = "customize"` without `tagged_vlan_mgmt = "custom"` now raises a warning, since the controller derives `forward` from `tagged_vlan_mgmt` (`custom` -> `customize`, `block_all` -> `native`) and ignores a direct write (#3)
...
- **`unifi_network`: a network deleted outside Terraform no longer breaks every subsequent plan.** `Read` reported the controller's 404 as an error diagnostic, so deleting a network in the UniFi UI turned each following `plan`/`apply` into a hard failure that could only be cleared with a manual `terraform state rm`. A not-found read now removes the resource from state, which is the standard refresh semantic: the network shows as drifted and is re-created on the next apply. Detection is narrowed to go-unifi's typed `*unifi.NotFoundError` (HTTP 404, or a read that returns no object for the ID) so genuine API failures still surface as errors and a live network is never silently destroyed and re-created (#4)
